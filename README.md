...
The default MySQL and PostgreSQL database name is `playground`. PostgreSQL uses `admin` as the superuser, the MySQL stack initializes both `root` and a local `admin` account with full privileges for local development, Portainer initializes the `admin` account with the password `adminadmin!!`, and RustFS exposes its S3 API on `http://127.0.0.1:9000`, `http://kimbank.local:9000`, and `http://ehkim.local:9000` plus the web console on `http://127.0.0.1:9001`, `http://kimbank.local:9001`, and `http://ehkim.local:9001` with `admin` / `adminadmin!!`. RustFS CORS is explicitly opened for `localhost`, `127.0.0.1`, `kimbank.local`, and `ehkim.local` on the API and console ports.
