...
- Keep the Homebrew runtime pin compatible with the current `homebrew/core` and `homebrew/cask` pins. A Nix build does not parse formula or cask DSL because `brew bundle` runs during activation, so validate newly coordinated pins with the pinned runtime when Homebrew adds DSL.
...
- Go uses the nixpkgs package in [`modules/shared/packages.nix`](modules/shared/packages.nix) as its machine-wide default. Prefer a project-local `mise.toml` when a repository needs a different Go version, leave `GOPATH` unset to use Go's `$HOME/go` default, and keep `$HOME/go/bin` in the Home Manager `sessionPath` so `go install` commands are available.
